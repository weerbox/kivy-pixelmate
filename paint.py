...
def distance(pos1, pos2):
    ...
    # return (pow((pos1[0] - pos2[0]), 2) + pow((pos1[1] - pos2[1]), 2)) ** 0.5
...
class Paint(StencilView):
    ...
    def fbo_create(self, size, tex=None):
        ...
        if self.fbo is None:
            ...
            self.fbo = Fbo(size=size, texture=tex)  # android crash, cant replace old fbo
            self.fbo.texture.mag_filter = 'nearest'
            self.fbo.texture.min_filter = 'nearest'
        ...
        if self.tool_fbo is None:
            ...
            self.tool_fbo = Fbo(size=size, texture=tool_tex)  # android crash, cant replace old fbo
            self.tool_fbo.texture.mag_filter = 'nearest'
            self.tool_fbo.texture.min_filter = 'nearest'
    ...
    def canvas_put_drawarea(self, texture=None):
        ...
        with self.canvas:
            ...
            for laybox in layer.LayerBox.boxlist:
                ...
                # if self.tool_buffer.enabled:
                #     self.canvas.add(self.tool_buffer.rect)
    ...
    def fbo_create_on_canvas(self, size=None, pos=(0, 0)):
        ...
        self.fbo.draw()
        self.canvas.ask_update()
        # self.layerTexture[0] = self.fbo.texture
    ...
    def fbo_skip_render(self, state):
        if state == True or state == 'down':
            self.fboSkipRender = True
        else:
            self.fboSkipRender = False


    def fbo_render(self, touch=None):
        # if not self.fboSkipRender:
        self.fbo.bind()
        if touch and touch.ud.has_key('line'):
            with self.fbo:
                self.fbo.add(self.app.aColor)
                d = 1.
                #self.cursorPos = ((touch.x-d/2)/self.scale - self.fboOffset[0], (touch.y-d/2)/self.scale - self.fboOffset[1])
                self.cursorPos = ((touch.x - d / 2) / self.scale - self.fbo_rect.pos[0],
                                  (touch.y - d / 2) / self.scale - self.fbo_rect.pos[1])

                #Ellipse(pos1=self.cursorPos, size=(d, d))
    ...
    def fbo_fill_region(self, touch):
        ...
        # improc.fillimage(self.fbo, (x, y), [0,0,0,0])
        #
        # self.fbo.bind()
        # improc.fill_texture(tex=self.fbo.texture, pos1=(x, y), color=[255, 0, 0])
        # self.fbo.release()
    ...
    def fbo_replace_color(self, touch):
        ...
        # self.fbo.bind()
        # improc.texture_replace_color(tex=self.fbo.texture, pos=(cx, cy), color=0, size=(3, 3))
        # # improc.new_color(self.fbo.texture, self.app.aColor, x, y, self.fbo)
        # self.fbo.release()
        # self.fbo.clear()
        # self.fbo.draw()
        # self.canvas.ask_update()
    ...
    def touch_responce(self):
        ...
        if len(self.touches) >= 2:
            ...
            print pos1, pos2, dist, self.touch_dist
        ...
        if self.app.active_tool == TOOL_PENCIL:
            ...
            if touch:
                if self.collide_point(touch.x, touch.y):
                    ...
                    # self.app.active_tool = self.app.prev_tool
    ...
    def on_touch_move(self, touch):
        ...
        if self.app.active_tool == TOOL_PENCIL:
            if self.app.dialogState == self.app.state['root']:
    ...
    def on_touch_up(self, touch):
        ...
        if touch:
            ...
            if self.app.active_tool == TOOL_SELECT:
                if touch:
                    if touch:
                        touch_pos = (touch.x / self.scale,
                                     touch.y / self.scale)
                    # self.tool_select.on_touch_up(touch_pos)
                    # self.tool_select.on_touch_up((touch.x, touch.y))
    ...
    def add_undo_stack(self):
        ...
        print "added to undo", len(self.layer_undo_stack)
    ...
    def do_undo(self, *args):
        if self.layer_undo_stack:
            if self.undo_layer_index > 0:
                ...
                # print(self.undo_layer_index, _layer._textures_array_index)
            self.fbo_update_pos()
